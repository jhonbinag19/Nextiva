...
module.exports = async (req, res) => {
  ...
  if (authResult.demo) {
    return json(res, 200, {
      ...
      data: {
        demo: true,
        consent: body,
        note: 'Demo Thrio commconsent: request not sent to Thrio'
      }
    });
  }

  const baseUrl = config.api.thrio.baseUrl;
  const url = `${baseUrl}/data/api/types/commconsent`;
  ...
  try {
    ...
    return json(res, status, {
      success: false,
      message: (error && error.response && error.response.data && error.response.data.message) || (error && error.message) || 'Failed to send commconsent',
